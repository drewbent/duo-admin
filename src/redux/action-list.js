export const HIDE_FLASH = 'HIDE_FLASH'
export const SHOW_FLASH = 'SHOW_FLASH'

export const SET_LOGGED_IN = 'SET_LOGGED_IN'
export const UPDATE_CURRENT_USER = 'UPDATE_CURRENT_USER'
export const LOG_OUT = 'LOG_OUT'

export const ADD_CLASSES = 'ADD_CLASSES'
export const SET_CLASSES = 'SET_CLASSES'
export const DELETE_CLASS = 'DELETE_CLASS'

export const SET_CLASS_STUDENTS = 'SET_CLASS_STUDENTS'
export const DELETE_CLASS_STUDENT = 'DELETE_CLASS_STUDENT'
export const ADD_CLASS_STUDENTS = 'ADD_CLASS_STUDENTS'

export const ADD_STUDENTS = 'ADD_STUDENTS'
export const DELETE_STUDENT = 'DELETE_STUDENT'

export const ADD_USERS = 'ADD_USERS'
export const SET_USERS = 'SET_USERS'
export const DELETE_USER = 'DELETE_USER'

export const SET_USER_COMPLETIONS = 'SET_USER_COMPLETIONS'

export const ADD_COMPLETIONS = 'ADD_COMPLETIONS'

export const ADD_SESSIONS = 'ADD_SESSIONS'

export const SET_CLASS_SESSIONS = 'SET_CLASS_SESSIONS'
export const ADD_CLASS_SESSIONS = 'ADD_CLASS_SESSIONS'

export const SET_SESSION_BEFORE = 'SET_SESSION_BEFORE'
export const SET_SESSION_AFTER = 'SET_SESSION_AFTER'

export const ADD_QUESTIONS = 'ADD_QUESTIONS'

export const SET_QUESTION_TYPES = 'SET_QUESTION_TYPES'

export const ADD_FORMS = 'ADD_FORMS'

export const ADD_FORM_QUESTIONS = 'ADD_FORM-QUESTIONS'

export const DELETE_DISTRIBUTION = 'DELETE_DISTRIBUTION'
export const ADD_DISTRIBUTIONS = 'ADD_DISTRIBUTIONS'

export const ADD_RESPONSES = 'ADD_RESPONSES'

export const SET_CANCELLATION_REASONS = 'SET_CANCELLATION_REASONS'

export const SET_SKILLS_FOR_CLASS = 'SET_SKILLS_FOR_CLASS'

export const ADD_MATCHING_ALGORITHMS = 'ADD_MATCHING_ALGORITHMS'

export const ADD_ACTIVE_MATCHING_ALGORITHMS = 'ADD_ACTIVE_MATCHING_ALGORITHMS'